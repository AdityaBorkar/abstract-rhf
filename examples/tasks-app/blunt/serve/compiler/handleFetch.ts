...
export async function handleFetch(req: Request, server: Server) {
	const { router, GLOBAL_CONFIG } = globalThis.BLUNTJS;
	...
	if (paths[1] === '_public') {
		return new Response(file(`./.blunt/${path.replaceAll('_public/', '')}`));
	}
	if (paths[1] === '_actions') {
	}
	...
	if (type === 'page')
		return CompilePage({ GLOBAL_CONFIG, request, server, files });
	if (type === 'route')
		return CompileRoute({ GLOBAL_CONFIG, request, server, files });
